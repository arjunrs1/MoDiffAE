...
def main():
    ...
    for ch in checkpoints:
        ...
        grade_maes = [0 if math.isnan(x) else x for x in grade_maes]
    ...
    eval_dir = os.path.join(args.save_dir, "evaluation_on_test_data")
    ...
    print(list(set(chosen_model_predictions_and_targets[0])))

    exit()'''


    grade_confusion_matrix_values = confusion_matrix(
        chosen_model_predictions_and_targets[1], chosen_model_predictions_and_targets[0], labels=list(grade_idx_to_name_short.keys())
    )

    print(grade_confusion_matrix_values)

    grade_confusion_matrix_values = grade_confusion_matrix_values[1, :]
    #grade_confusion_matrix_values = np.swapaxes(grade_confusion_matrix_values, 0, 1)
    grade_confusion_matrix_values = np.expand_dims(grade_confusion_matrix_values, 0)

    print(grade_confusion_matrix_values)

    # print(grade_confusion_matrix_values)
    df_cm = pd.DataFrame(grade_confusion_matrix_values,
                         index=[grade_idx_to_name_short[1]],
    ...
    s = sns.heatmap(df_cm, annot=True, cmap='Blues', cbar=False)
