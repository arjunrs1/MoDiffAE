...
def create_multi_index_cols(labels): 
    column_combinations = [('Time', '')]
    for l in labels:
        if l != 'Time': 
...
def center_initial_position(event_dfs): 
    centered_event_dfs = []
    for e_df in event_dfs: 
        e_df = e_df.copy()
...
def main(desired_frequency, data_dir, output_dir, replace):
    ...
    for _, file_name in zip(tqdm(range(number_of_files), desc='Processing motion data'), file_names):
        ...
        if missing_LTHI:
            ...
            event_dfs = center_initial_position(event_dfs)
            add_to_sample_list(
                sample_list, 
                event_dfs, 
                attacker_code, 
    ...
    if len(missing_LTHI_sample_list) > 0:
        ...
        for _, (e_dfs, a_code, tech_cls, cond) in zip(tqdm(range(len(missing_LTHI_sample_list)), 
            ...
            event_dfs = center_initial_position(completed_dfs)
            add_to_sample_list(
                sample_list, 
                completed_dfs, 
                a_code, 
...
def get_args():
    ...
    parser.add_argument('--desired_frequency', '-f', dest='desired_frequency', type=int, default=50,
